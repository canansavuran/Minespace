...
namespace Minespace
{
    public partial class TumOyunlarPage : PhoneApplicationPage
    {

        public static KayitliOyun deneme;
        

        public TumOyunlarPage()
        {
            InitializeComponent();
            List<String> OyunListem = MainPage.TumOyunlar;
            List<Button> butonlar = new List<Button>();

            for (int i = 0; i < OyunListem.Count && i < 10; i++)
            {
                Button yeni = new Button();
                yeni.Content = OyunListem[i];
                yeni.Height = 70;
                yeni.Width = 700;
                yeni.Click += Oyunlar_Click;
                butonlar.Add(yeni);
            }
        ...
        void Oyunlar_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (KayitliOyun oyunumm in oyunlar)
            {
                if (oyunumm.OyunAdi == ((Button)sender).Content.ToString())
                {
                    ...
                    Uri Uri = new Uri(String.Format("/TumOyunSecPage.xaml"), UriKind.Relative);
        ...
        private void OyunKaydet_Click_1(object sender, RoutedEventArgs e)
        {
            Uri Uri = new Uri(String.Format("/CreateGame.xaml"), UriKind.Relative);
            NavigationService.Navigate(Uri);
        }
